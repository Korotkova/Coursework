...
public class NorthwestCorner {
    ...
    public void SLedyushi(String parakey) {
        ...
        if (!poiskBSleva(IC, JC).equals("null") & !poiskBSleva(IC, JC).equals(parent)) {
           // System.out.println("слева зашел");
      ...
      //  System.out.println("итог "+pytuperem);
    ...
    for (int i = 0; i < columns; i++) {
        ...
        for (int i = 0; i < xyT.length; i += 2) {
            if (xMN[xyT[i]][xyT[i + 1]] == 0 & !flagNashel) {
                ...
                vonJ = xyT[i + 1];
                flagNashel = true;
                //System.out.println("нашёл первую нулевую");
            }
            if (xMN[xyT[i]][xyT[i + 1]] == 0 & flagNashel & (vonI != xyT[i] | vonJ != xyT[i + 1])){//если у нас несколько обнулится со знаком минус 
            //но они все равно базисные нужно им присвоить значение, чтобы их не потерять!
            //System.out.println("вдруг ноль");
